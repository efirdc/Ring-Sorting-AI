...
def rand_cmap(nlabels, type='bright', first_color=(0, 0, 0), verbose=True):
    """
    Creates a random colormap to be used together with matplotlib. Useful for segmentation tasks
    :param nlabels: Number of labels (size of colormap)
    :param type: 'bright' for strong colors, 'soft' for pastel colors
    :param first_color: Option to for first color
...
class RingPlot:
    def __init__(self, n, cmap="random", cmap_seed=1):
        if cmap == "random":
            np.random.seed(cmap_seed)
            self.cmap = rand_cmap(n + 1, type='bright', first_color=[0, 0, 0], verbose=False)
        else:
            self.cmap = cmap
